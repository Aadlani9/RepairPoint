    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        switch ($action) {
            case 'add_user':
                ...
                $message = $success ? 'تم إضافة المستخدم بنجاح' : 'خطأ في إضافة المستخدم';
                break;
                
            case 'edit_user':
                $success = editUser($_POST, $shop_id);
                $message = $success ? 'تم تحديث المستخدم بنجاح' : 'خطأ في تحديث المستخدم';
                break;
                
            case 'toggle_status':
                $success = toggleUserStatus($_POST['user_id'], $shop_id);
                $message = $success ? 'تم تحديث حالة المستخدم' : 'خطأ في تحديث الحالة';
                break;
                
            case 'delete_user':
                $success = deleteUser($_POST['user_id'], $shop_id, $current_user['id']);
                $message = $success ? 'تم حذف المستخدم بنجاح' : 'خطأ في حذف المستخدم';
                break;
        }
...
function addNewUser($data, $shop_id) {
    ...
    try {
        ...
        if ($user_id) {
            logActivity('user_created', "مستخدم جديد: {$data['name']} ({$data['email']})", $_SESSION['user_id']);
            return true;
        }
    } catch (Exception $e) {
        error_log("خطأ في إضافة مستخدم: " . $e->getMessage());
    }
...
function editUser($data, $shop_id) {
    ...
    try {
        ...
        if ($updated !== false) {
            logActivity('user_updated', "تحديث مستخدم: {$data['name']}", $_SESSION['user_id']);
            return true;
        }
    } catch (Exception $e) {
        error_log("خطأ في تحديث مستخدم: " . $e->getMessage());
    }
...
function toggleUserStatus($user_id, $shop_id) {
    ...
    try {
        ...
        if ($updated !== false) {
            logActivity('user_status_changed', "تغيير حالة المستخدم {$user['name']} إلى $new_status", $_SESSION['user_id']);
            return true;
        }
    } catch (Exception $e) {
...
function deleteUser($user_id, $shop_id, $current_user_id) {
    ...
    try {
        ...
        if ($deleted) {
            $db->commit();
            logActivity('user_deleted', "حذف المستخدم: {$user['name']}", $current_user_id);
            return true;
        } else {
            $db->rollback();
        }
...
<div class="container-fluid">
                ...
                <div class="row align-items-center">
                    <div class="col-md-8">
                        ...
                        <p class="mb-0 opacity-75">
                            إدارة فريق العمل وصلاحيات المستخدمين
                        </p>
                    </div>
    ...
    <div class="card mb-4">
        <div class="card-body">
            ...
            <div class="mt-3">
                <span class="text-muted">الفلاتر النشطة:</span>
                <?php if ($search): ?>
                    <span class="badge bg-primary me-2">البحث: <?= htmlspecialchars($search) ?></span>
                <?php endif; ?>
                <?php if ($role_filter): ?>
                    <span class="badge bg-secondary me-2">الدور: <?= $role_filter === 'admin' ? 'مدير' : 'موظف' ?></span>
                <?php endif; ?>
                <?php if ($status_filter): ?>
                    <span class="badge bg-secondary me-2">الحالة: <?= $status_filter === 'active' ? 'نشط' : 'غير نشط' ?></span>
                <?php endif; ?>
                <a href="<?= url('pages/users.php') ?>" class="btn btn-sm btn-outline-secondary">
                    <i class="bi bi-x me-1"></i>مسح الفلاتر
                </a>
            </div>
            <?php endif; ?>
        </div>
            ...
            <?php if (empty($users)): ?>
                <div class="text-center p-5">
                    ...
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addUserModal">
                        <i class="bi bi-person-plus me-2"></i>إضافة أول مستخدم
                    </button>
                </div>
            <?php else: ?>
                            ...
                            <tr class="user-row <?= $user['status'] === 'inactive' ? 'table-secondary' : '' ?>">
                                ...
                                <td>
                                    <div class="repairs-stats">
                                        ...
                                        <small class="text-muted">إصلاح</small>
                                    </div>
                                </td>
                                <td>
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="" class="needs-validation" novalidate>
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="add_name" class="form-label">الاسم الكامل *</label>
                        <input type="text" 
                               class="form-control" 
                               id="add_name" 
                               name="name" 
                    ...
                    <div class="mb-3">
                        <label for="add_role" class="form-label">الدور *</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save me-2"></i>إضافة المستخدم
                    </button>
                </div>
            </form>
        </div>
            ...
            <form method="POST" action="" class="needs-validation" novalidate id="editUserForm">
                <div class="modal-body">
                    ...
                    <div class="alert alert-info">
                        ...
                        لتغيير كلمة المرور، يجب على المستخدم تغييرها من صفحة الملف الشخصي
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إلغاء</button>
